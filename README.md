...
- 設定前の研究利用確認と、「はい」を選んだ場合だけ行う匿名JSON記録
...
- 設定完了後、実際のモンスター・障害物・ボス弾を使って3つの対処動作を試す子ども向けチュートリアル
- 肩幅・胴体長・脚長など、カメラ画面内での相対的な身体サイズの確認
- 練習時の跳び幅に合わせた個人別のジャンプ判定
- センサーありモードでは、MediaPipeの腰・足首の上昇と肩甲骨付近のIMU加速度を組み合わせた両脚ジャンプ判定
- ジャンプ後の着地判定
- 足元に表示された敵を踏み潰す演出
- 両肩と両腰の位置を使った横曲げ判定
- センサーありモードでは、MediaPipeの横曲げ方向とIMUの重力ベクトルから求めた上体の傾きを組み合わせた回避判定
- 左右から接近する障害物と回避演出
...
M5StickC PLUS2を併用します。両脚ジャンプではMediaPipeの骨格とIMUの加速度、
横曲げ回避ではMediaPipeの方向とIMUの傾き、ボス戦のねじり反射ではMediaPipeの
姿勢とIMUのジャイロを組み合わせます。
IMUが未接続でもMediaPipe判定でゲームを続行できます。
...
1. 研究利用の確認で「はい」または「いいえ」を選びます。「いいえ」でも同じ内容で遊べます。
2. カメラに全身を映します。
3. 静止して、カメラ上の身体サイズと基準姿勢を測定します。
4. 画面の案内に従い、体のねじり、両脚跳び、斜め下への曲げと胸反らしを行います。
5. 測定結果を確認して「チュートリアルへ」を押します。
6. チュートリアルで、紫のモンスターを両脚ジャンプと着地で踏み潰します。
7. 左から来る黄色い障害物に対して、上体を右へ曲げて回避します。
8. 近づくボスのエネルギー弾に対して、脚を動かさず上体をねじって反射します。
9. 3種類に成功したら「本番スタート！」を押します。
10. 本番では、表示された敵や障害物に合わせて3つの動きを使い分けます。
11. バトル開始から3分後に、得点・撃破・回避・反射の結果が表示されます。途中で「ここまでの結果を見る」を押すこともできます。
12. 結果画面は10秒後に自動でトップ画面へ戻ります。

## 研究用データの記録

設定画面より前に研究利用の確認を表示します。「いいえ」を選んだ場合は時系列データを
作成せず、そのまま通常のゲームへ進みます。「はい」を選んだ場合だけ、次を約10Hzで
ブラウザのメモリへ記録します。

- MediaPipeの33個の正規化骨格座標（x、y、z、visibility）
- M5StickC PLUS2の3軸加速度と3軸ジャイロ
- 設定、チュートリアル、本番などのフェーズと経過時刻
- ジャンプ攻撃、障害物回避、ボス弾反射の成否
- 個人調整後の判定値と最終スコア

カメラ映像、音声、氏名、IMU端末IPは記録しません。データにはランダムなセッションIDを
付け、本番終了時に `radio-heroes-research-日時.json` としてプレイ中のPCへ保存します。
結果画面の「研究データをもう一度保存」から再保存もできます。Vercelや外部データベースへ
自動送信する処理はありません。トップ画面へ戻るとメモリ上の記録を破棄し、次のプレイヤーに
改めて同意を確認します。

文化祭で研究データを収集する前に、研究目的、研究責任者、問い合わせ先、保存期間、撤回・削除方法、
未成年者の同意方法を所属機関の研究倫理審査方針に合わせて確定してください。画面の確認だけで正式な
インフォームド・コンセントの要件を満たすとは限りません。

### チュートリアルフェーズ

ヒーロースキャンと本番の間に3段階の体験型チュートリアルを表示します。各段階では
本番と同じ画面オブジェクトを動かし、対象物、行う動き、成功結果を大きな日本語で
案内します。チュートリアル中の成功は本番の得点・撃破数・回避数・反射数には
加算せず、3分間のバトルタイマーも開始しません。

判定方法は本番と同じです。「センサーあり」ではMediaPipeとIMUを組み合わせ、
IMUが未接続または古い場合はMediaPipeのみへ切り替えます。3種類すべてに成功すると
「本番スタート！」ボタンが表示され、押した時点から3分間の本番が始まります。
...
「センサーあり」では、さらにIMUの3軸加速度から合成値を求め、静止時の
重力加速度との差をジャンプ衝撃量として使います。軸の向きを固定せず合成値を
使うため、肩甲骨への装着角度が多少変わっても判定できます。

```text
加速度合成値 = √(ax² + ay² + az²)
IMUジャンプ強度 = |加速度合成値 − 静止時合成値|
```

ヒーロースキャン中の2回の両脚ジャンプでIMU強度の最大値を測り、その42%と
静止時のぶれを考慮した値の大きい方を個人別しきい値にします。バトル中は、
MediaPipeで「腰と両足首が上昇した」ことと、IMUで「加速度変化がしきい値を
超えた」ことがほぼ同時に成立した場合だけジャンプ開始とします。カメラ検出は
520ミリ秒、IMU検出は700ミリ秒保持し、両機器の通信や描画のずれを吸収します。
IMUが未調整または途切れた場合は、MediaPipeだけの従来判定に自動で切り替わります。
...
「センサーあり」では、静止時のIMU加速度3軸を基準重力ベクトルとし、現在の
平滑化した加速度ベクトルとの角度差を求めます。3軸全体の角度差を使うため、
M5StickC PLUS2の装着方向が少し変わっても使用できます。

```text
IMU傾き角 = arccos((現在の重力ベクトル · 基準重力ベクトル)
                    ÷ (|現在ベクトル| × |基準ベクトル|))
```

ヒーロースキャンの「体を斜め下に曲げる運動」で最大傾き角を測り、その34%を
回避確認用の個人別しきい値にし、7～16度に収めます。実際の回避では、MediaPipeが
正しい左右方向を確認し、同時にIMU傾き角がしきい値以上になった場合に成功です。
IMU検出は900ミリ秒保持し、カメラとIMUの受信時刻の小さなずれを吸収します。
